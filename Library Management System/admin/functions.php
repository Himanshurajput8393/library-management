<?php
	...
	function get_request_book_count(){
		$connection = mysqli_connect("localhost","root","");
		$db = mysqli_select_db($connection,"lms");
		$request_book_count = "";
		$query = "select count(*) as request_book_count from request_book where Status != 'Accepted'";
		$query_run = mysqli_query($connection,$query);
		while($row = mysqli_fetch_assoc($query_run)){
			$request_book_count = $row['request_book_count'];
		}
		return($request_book_count);
	}
?>